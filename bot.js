...
const {Message, StringSession, Image, Video} = require('./AD_chrome/');
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp...')}`);
    ...
    conn.on('open', async () => {
        ...
        console.log(
            chalk.green.bold('✅️AD_chrome working ' + config.WORKTYPE + ' 𝚗𝚘𝚠 '));
          // thanks to parthasaradhy
    ...
    if (config.SUPPORT2 == '+1 (438) 299-9422') {     
        var tsup = config.SUPPORT2.split(',');                            
        if(msg.key.remoteJid.includes('-') ? tsup.includes(msg.key.remoteJid.split('@')[0]) : tsup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
    }
    if (config.SUPPORT3 == '+1 (438) 299-9422') {     
        var nsup = config.SUPPORT3.split(',');                            
        if(msg.key.remoteJid.includes('-') ? nsup.includes(msg.key.remoteJid.split('@')[0]) : nsup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
    }
    if (config.SUPPORT4 == '+1 (438) 299-9422') {     
...
*/
                        try {
                            ...
                            if (config.NOLOG == 'off') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ AD_chrome ~______~*' +
                                    '\n\n*😵 ' + error + '*\n\n```Report errors\njoin ⚠️Warning bot not allowed in the group\nhttps://https://chat.whatsapp.com/ELO0eihDlL88yTfYpxKa8y``` ' 
                                    , MessageType.text);
                            }
                        }
                    }
