...
Asena.addCommand({on: 'text', fromMe: wk, dontAddCommandList: true, deleteCommand: false}, (async (message, match) => {
    if (message.message.startsWith('ADchrome') && conf.TALKING_ADchrome !== 'true') {        
        ...
        var finm = message.message.replace('ADchrome', '').replace(' ', '')   
...
var succ_off = ''
if (conf.LANG == 'TR') {
    fulleva_dsc = 'Tam fonksiyonel ADchrome özelliklerini aktif eder. Hesabınızı bir chatbota dönüştürün!'
    already_on = 'ADchrome yapay zekası halihazırda tüm fonksiyonları etkin.'
    already_off = 'ADchrome yapay zekası halihazırda yarı fonksiyonel çalışıyor.'
    succ_on = 'ADchrome, Tam Fonksiyonel Olarak Açıldı! Lütfen Biraz Bekleyin! ✅'
    succ_off = 'ADchrome, Yarı Fonksiyonel Olarak Ayarlandı! Lütfen Biraz Bekleyin! ☑️'
}
if (conf.LANG == 'EN') {
    fulleva_dsc = 'Activates full functional ADchrome features. Turn your account into a ai chatbot!'
    already_on = 'ADchrome artificial intelligence is already fully functional.'
    already_off = 'ADchrome artificial intelligence is currently running semi-functional.'
    succ_on = 'ADchrome Opened Fully Functionally! Please wait a bit! ✅'
    succ_off = 'ADchrome Set to Semi-Functional! Please wait a bit! ☑️'
}
if (conf.LANG == 'ML') {
    fulleva_dsc = 'പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായ ADchrome സവിശേഷതകൾ സജീവമാക്കുന്നു. നിങ്ങളുടെ അക്കൗണ്ട് ഒരു ചാറ്റ്ബോട്ടാക്കി മാറ്റുക!'
    already_on = 'ADchrome കൃത്രിമബുദ്ധി ഇതിനകം പൂർണ്ണമായി പ്രവർത്തിക്കുന്നു.'
    already_off = 'ADchrome AI നിലവിൽ സെമി-ഫംഗ്ഷണൽ ആണ്.'
    succ_on = 'ADchrome പൂർണ്ണമായും പ്രവർത്തനക്ഷമമായി തുറന്നു! കുറച്ച് കാത്തിരിക്കൂ! ✅'
    succ_off = 'സെമി-ഫങ്ഷണൽ ആയി ADchrome സജ്ജമാക്കുക! കുറച്ച് കാത്തിരിക്കൂ! ☑️'
}

Asena.addCommand({ pattern: 'chrome ?(.*)', desc: fulleva_dsc, fromMe: true,dontAddCommandList: true, usage: '.chrome on / off' }, (async (message, match) => {
    var chrome_status = `${conf.TALKING_ADchrome}`
    if (match[1] == 'on') {
        if (chrome_status == 'true') {
            return await message.client.sendMessage(message.jid, '*' + already_on + '*', MessageType.text)
        }
        else {
            await heroku.patch(baseURI + '/config-vars', { 
                body: { 
                    ['TALKING_ADchrome']: 'true'
                } 
            });
